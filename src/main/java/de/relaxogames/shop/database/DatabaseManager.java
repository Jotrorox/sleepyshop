...
public class DatabaseManager {
    ...
    public CompletableFuture<Void> saveShop(Shop shop) {
        ...
        return CompletableFuture.runAsync(() -> {
            String sql = "INSERT OR REPLACE INTO shops (sign_location, chest_location, owner, sell_item, payment_item, " +
                    "take_amount, output_amount, shop_name, show_display, show_stock_message, display_entity_id) " +
                    "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
            try (PreparedStatement pstmt = connection.prepareStatement(sql)) {
                pstmt.setString(1, serializeLocation(shop.getSignLocation()));
                pstmt.setString(2, serializeLocation(shop.getChestLocation()));
                pstmt.setString(3, shop.getOwner().toString());
                pstmt.setString(4, serializeItemStack(shop.getSellItem()));
                pstmt.setString(5, serializeItemStack(shop.getPaymentItem()));
    ...
    public CompletableFuture<List<Shop>> loadShops() {
        if (connection == null) return CompletableFuture.completedFuture(new ArrayList<>());
        return CompletableFuture.supplyAsync(() -> {
            ...
            try (Statement stmt = connection.createStatement();
                ...
                while (rs.next()) {
                    Location signLoc = deserializeLocation(rs.getString("sign_location"));
                    Location chestLoc = deserializeLocation(rs.getString("chest_location"));
                    if (signLoc == null || chestLoc == null) continue;
                    
                    UUID owner = UUID.fromString(rs.getString("owner"));
                    
                    Shop shop = new Shop(signLoc, chestLoc, owner);
                    shop.setSellItem(deserializeItemStack(rs.getString("sell_item")));
                    shop.setPaymentItem(deserializeItemStack(rs.getString("payment_item")));
