package com.jotrorox.shop.util;

import com.google.gson.JsonObject;
import com.google.gson.JsonParser;
import com.jotrorox.shop.listener.UpdateCheckListener;
...
public class UpdateChecker {
    ...
    public CompletableFuture<VersionResult> checkHangar() {
        return CompletableFuture.supplyAsync(() -> {
            try {
                String apiUrl = "https://hangar.papermc.io/api/v1/projects/" + hangarSlug + "/versions";
                String response = makeRequest(apiUrl);

                if (response == null) {
                    return new VersionResult(false, null, "Failed to connect to Hangar");
                }
                ...
                if (json.has("result") && !json.getAsJsonArray("result").isEmpty()) {
                    ...
                    boolean isOutdated = compareVersions(currentVersion, latestVersion) < 0;

                    return new VersionResult(isOutdated, latestVersion, downloadUrl);
                }

                return new VersionResult(false, null, "No versions found");

            } catch (Exception e) {
                plugin.getLogger().warning("Failed to check for updates on Hangar: " + e.getMessage());
                return new VersionResult(false, null, "Error: " + e.getMessage());
            }
        });
    }
    ...
    public void performCheck() {
        ...
        checkHangar().thenAccept(result -> {
            if (result.outdated()) {
                plugin.getLogger().warning("A new version is available on Hangar!");
                plugin.getLogger().warning("Current: " + currentVersion + " | Latest: " + result.latestVersion());
                plugin.getLogger().warning("Download: " + result.downloadUrl());

                plugin.getServer().getPluginManager().registerEvents(new UpdateCheckListener(plugin, result), plugin);
            } else if (result.latestVersion() != null) {
                plugin.getLogger().info("You are running the latest version from Hangar!");
            }
        });
    }
    ...
    public record VersionResult(boolean outdated, String latestVersion, String downloadUrl) {}
}